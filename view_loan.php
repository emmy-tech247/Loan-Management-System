...
$workflow = [
    "submitted"            => ["next_status" => "forwarded_to_officer", "next_role" => "relationship_officer", "label" => "Forward to Relationship Officer"],
    "forwarded_to_officer" => ["next_status" => "forwarded_to_auditor", "next_role" => "auditor",              "label" => "Forward to Auditor"],
    "forwarded_to_auditor" => ["next_status" => "forwarded_to_manager", "next_role" => "manager",              "label" => "Forward to Manager"],
    "forwarded_to_manager" => ["next_status" => "forwarded_to_accountant", "next_role" => "accountant",       "label" => "Forward to Accountant"],
    "forwarded_to_accountant" => ["next_status" => "approved", "next_role" => "accountant", "label" => "Mark as Approved ✅"],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        ...
        if (isset($workflow[$current_status])) {
            ...
            $msg = "✅ Loan " . $next['label'] . ".";
            // Refresh loan info
            header("Location: view_loan.php?id=" . $loan_id . "&msg=" . urlencode($msg));
            exit;
        } else {
            $msg = "⚠️ This loan is already finalized.";
        }
...
// Optional message passed after redirect
if (isset($_GET['msg'])) {
    $msg = $_GET['msg'];
}
?>
...
<head>
  ...
  <style>
    ...
    .card th { width: 200px; font-weight: 600; color: #555; }
