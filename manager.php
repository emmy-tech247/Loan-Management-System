...
// ✅ Fetch loans approved by Auditor and forwarded to Manager
$sql = "SELECT l.loan_id, l.loan_amount, l.tenure_month, l.loan_status, l.created_at, 
               CONCAT(m.surname, ' ', m.first_name, ' ', m.other_names) AS full_name
        FROM loans l
        INNER JOIN members m ON l.member_id = m.member_id
        WHERE l.loan_status = 'forwarded_to_manager'
...
<body>
  ...
  <h2>Manager Dashboard – Loans Awaiting Final Approval</h2>
  <table id="loanTable">
    <tr>
      <th>Loan ID</th>
    ...
    <?php if ($result && $result->num_rows > 0): ?>
      <?php while($row = $result->fetch_assoc()): ?>
        <tr>
          ...
          <td><a class="review-btn" href="manager_review_loan.php?id=<?= $row['loan_id'] ?>">Review</a></td>
        </tr>
      <?php endwhile; ?>
    <?php else: ?>
      <tr><td colspan="7">No loans awaiting manager approval.</td></tr>
    <?php endif; ?>
    </tbody>
  </table>

  <script>
    // ✅ Auto-refresh loan list without reloading page
    function fetchLoans() {
      fetch("manager_fetch_loans.php")
