...
// === Workflow for Auditor (forward to Manager) ===
$workflow = [
    "forwarded_to_auditor" => ["next_status" => "forwarded_to_manager", "label" => "Forward to Manager"],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve' && isset($workflow[$loan['loan_status']])) {
        ...
        $update = $conn->prepare("UPDATE loans SET loan_status=? WHERE loan_id=?");
        $update->bind_param("si", $next['next_status'], $loan_id);
        $update->execute();
        $msg = "✅ Loan " . $next['label'] . ".";
        header("Location: auditor_review_loan.php?id=$loan_id&msg=" . urlencode($msg));
        exit;
    } elseif ($action === 'reject') {
        $update = $conn->prepare("UPDATE loans SET loan_status='rejected' WHERE loan_id=?");
...
<head>
  <meta charset="UTF-8">
  <title>Auditor Loan Review</title>
  <style>
...
<body>
  <div class="container">
    <h2>Review Loan #<?= htmlspecialchars($loan['loan_id']) ?> (Auditor)</h2>
    ...
    <div class="actions">
      ...
      <a href="auditor_dashboard.php" class="back">⬅ Back to Dashboard</a>
    </div>
  </div>
</body>
</html>
